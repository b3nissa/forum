	...
	if ($display_style == "div") {
		...
		<ul class="vmproduct<?php echo $params->get ('moduleclass_sfx'); ?> productdetails">
			...
			<li class="product-container <?php echo $pwidth ?> <?php echo $float ?> ">
				...
				echo '<div class="clear"></div>';
				$url = JRoute::_ ('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' . $product->virtuemart_product_id . '&virtuemart_category_id=' .$product->virtuemart_category_id); ?>
				<a class="product-titel-item" href="<?php echo $url ?>"><?php echo $product->product_name ?></a>
				...
				if ($show_addtocart) {
					...
					if ($show_price and  isset($product->prices)) {
						...
						foreach ($product->customfieldsSorted['normal'] as $optie) {
						   if ($optie->custom_title == "van prijs") {
							   $vanprijs = $optie->customfield_value;
						   }

							 if ($product->product_lwh_uom == "CM") {
								 $lengte = $product->product_length;
								 $lengte2 = $lengte*1;
							 }
					   }

							echo "<div class='vanprijs'>

							per lengte van ". $lengte2 ."cm<br /><span>".$currency->priceDisplay($vanprijs)."

						</span></div>";

						$prijspermeter = ($product->prices['salesPrice'] / $product->product_length);
						if ($product->product_lwh_uom == "CM") {
							$prijspermeter = $prijspermeter*100;
							echo "
							<div class='prijs-pm'>
								Per meter <br />
								<span>".$currency->priceDisplay($prijspermeter)."</span>
							</div>
							";
